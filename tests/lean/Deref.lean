...
/-- [deref::use_deref_vec]:
    ...
    Visibility: public -/
def use_deref_vec {T : Type} (x : alloc.vec.Vec T) : RustM (Slice T) := do
...
def use_deref_mut_vec
  ...
  RustM ((Slice T) × (Slice T → alloc.vec.Vec T))
  := do
