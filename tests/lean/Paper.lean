-- THIS FILE WAS AUTOMATICALLY GENERATED BY AENEAS
-- [paper]
import Aeneas
open Aeneas Aeneas.Std RustM ControlFlow Error
set_option linter.dupNamespace false
set_option linter.hashCommand false
set_option linter.unusedVariables false

/- You can set the `maxHeartbeats` value with the `-max-heartbeats` CLI option -/
set_option maxHeartbeats 1000000

/- You can set the `maxRecDepth` value with the `-max-recdepth` CLI option -/
set_option maxRecDepth 2048

namespace paper

/-- [paper::ref_incr]:
    Source: 'tests/src/paper.rs', lines 7:0-9:1
    Visibility: public -/
def ref_incr (x : Std.I32) : RustM Std.I32 := do
  x + 1#i32

/-- [paper::test_incr]:
    Source: 'tests/src/paper.rs', lines 12:0-16:1
    Visibility: public -/
def test_incr : RustM Unit := do
  let x ← ref_incr 0#i32
  massert (x = 1#i32)

/- Unit test for [paper::test_incr] -/
#assert (test_incr == ok ())

/-- [paper::choose]:
    Source: 'tests/src/paper.rs', lines 19:0-25:1
    Visibility: public -/
def choose
  {T : Type} (b : Bool) (x : T) (y : T) : RustM (T × (T → (T × T))) := do
  if b
  then let back := fun x1 => (x1, y)
       ok (x, back)
  else let back := fun y1 => (x, y1)
       ok (y, back)

/-- [paper::test_choose]:
    Source: 'tests/src/paper.rs', lines 28:0-36:1
    Visibility: public -/
def test_choose : RustM Unit := do
  let (z, choose_back) ← choose true 0#i32 0#i32
  let z1 ← z + 1#i32
  massert (z1 = 1#i32)
  let (x, y) := choose_back z1
  massert (x = 1#i32)
  massert (y = 0#i32)

/- Unit test for [paper::test_choose] -/
#assert (test_choose == ok ())

/-- [paper::List]
    Source: 'tests/src/paper.rs', lines 40:0-43:1
    Visibility: public -/
@[discriminant isize]
inductive List (T : Type) where
| Cons : T → List T → List T
| Nil : List T

/-- [paper::list_nth_mut]:
    Source: 'tests/src/paper.rs', lines 47:0-60:1
    Visibility: public -/
def list_nth_mut
  {T : Type} (l : List T) (i : Std.U32) : RustM (T × (T → List T)) := do
  match l with
  | List.Cons x tl =>
    if i = 0#u32
    then let back := fun t => List.Cons t tl
         ok (x, back)
    else
      let i1 ← i - 1#u32
      let (x1, list_nth_mut_back) ← list_nth_mut tl i1
      let back := fun t => let tl1 := list_nth_mut_back t
                           List.Cons x tl1
      ok (x1, back)
  | List.Nil => fail panic
partial_fixpoint

/-- [paper::sum]:
    Source: 'tests/src/paper.rs', lines 62:0-71:1
    Visibility: public -/
def sum (l : List Std.I32) : RustM Std.I32 := do
  match l with
  | List.Cons x tl => let i ← sum tl
                      x + i
  | List.Nil => ok 0#i32
partial_fixpoint

/-- [paper::test_nth]:
    Source: 'tests/src/paper.rs', lines 74:0-79:1
    Visibility: public -/
def test_nth : RustM Unit := do
  let (x, list_nth_mut_back) ←
    list_nth_mut (List.Cons 1#i32 (List.Cons 2#i32 (List.Cons 3#i32 List.Nil)))
      2#u32
  let x1 ← x + 1#i32
  let l := list_nth_mut_back x1
  let i ← sum l
  massert (i = 7#i32)

/- Unit test for [paper::test_nth] -/
#assert (test_nth == ok ())

/-- [paper::call_choose]:
    Source: 'tests/src/paper.rs', lines 82:0-88:1
    Visibility: public -/
def call_choose (p : (Std.U32 × Std.U32)) : RustM Std.U32 := do
  let (px, py) := p
  let (pz, choose_back) ← choose true px py
  let pz1 ← pz + 1#u32
  let (px1, _) := choose_back pz1
  ok px1

end paper
