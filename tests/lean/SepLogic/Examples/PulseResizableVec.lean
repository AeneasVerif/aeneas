...
def push (v : ResizableVec α) (value : α) : Result Bool := do
  ...
  if size < cap then
    ...
    SepLogic.update v.sizeCell (size + 1)
    pure true
  else
    pure false
...
def pop (v : ResizableVec α) : Result (Option α) := do
  ...
  if size = 0 then
    ...
    SepLogic.update v.sizeCell last
    pure slot.join
...
def free (v : ResizableVec α) : Result Unit := do
  ...
  SepLogic.free v.sizeCell
  SepLogic.free v.capacityCell
