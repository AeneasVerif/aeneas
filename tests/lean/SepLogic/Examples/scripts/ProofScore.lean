...
With no file arguments it scores every file of `SepLogic/Examples`, and
writes `SepLogic/Examples/reports/proof-score.html`.
...
def renderReport (files : Array FileScore) : String := Id.run do
  ...
  out := out ++ "<p>Regenerate with <code>lake env lean --run \
    SepLogic/Examples/scripts/ProofScore.lean</code> from \
    ...
    <code>SepLogic/Examples/scripts/ProofScore.lean</code> for the details.</p>"
...
structure Options where
  ...
  out : System.FilePath := "SepLogic/Examples/reports/proof-score.html"

def usage : String :=
  "usage: lake env lean --run SepLogic/Examples/scripts/ProofScore.lean \
    [-o REPORT.html] [FILE.lean …]"
...
def defaultFiles : IO (Array System.FilePath) := do
  let dir : System.FilePath := "SepLogic/Examples"
...
end SepLogic.ProofScore

unsafe def main (args : List String) : IO UInt32 := SepLogic.ProofScore.main args
