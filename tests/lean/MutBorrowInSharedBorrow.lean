...
axiom core.slice.Slice.first {T : Type} : Slice T → RustM (Option T)
...
def MutFieldAccessViaShared.get
  (self : MutFieldAccessViaShared) : RustM Std.U32 := do
  ok self.data
...
def use_mut_field_access_via_shared : RustM Unit := do
...
def SliceWrapper.len {T : Type} (self : SliceWrapper T) : RustM Std.Usize := do
...
def SliceWrapper.first
  {T : Type} (self : SliceWrapper T) : RustM (Option T) := do
...
def use_slice_wrapper : RustM Unit := do
  let (s, _) ←
...
def use_slice_wrapper_param
  (buf : Slice Std.U32) : RustM (Std.Usize × (Slice Std.U32)) := do
...
def MixedBorrows.shared_len
  {T : Type} (self : MixedBorrows T) : RustM Std.Usize := do
...
def use_mixed_borrows : RustM Unit := do
...
def use_mixed_borrows_param
  ...
  RustM ((Std.Usize × Std.Usize) × (Slice Std.U32))
  := do
