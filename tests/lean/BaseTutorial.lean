...
def mul2_add1 (x : U32) : RustM U32 := do
  let x1 ← x + x
  let x2 ← x1 + 1#u32
  ok x2
...
/- # Syntax
   ...
   the [RustM] type. In particular, addition can fail.
   ...
   [x : RustM ...] successfully evaluates to some value, in which case it
...
/- We show a desugared version of [mul2_add1] below: we remove the syntactic
   ...
   results explicit.
 -/
def mul2_add1_desugared (x : U32) : RustM U32 :=
...
def use_mul2_add1 (x : U32) (y : U32) : RustM U32 := do
  let x1 ← mul2_add1 x
  x1 + y
...
/- Recursive functions may not terminate on all inputs.
   ...
   All you need to know is that the [RustM] type has actually 3 cases (we saw
