...
axiom wipe_slice {T : Type} : Slice T → RustM (Slice T)
...
/-- [drop_bug::{impl core::ops::drop::Drop for drop_bug::Inner}::drop]:
    ...
    Visibility: public -/
def Inner.Insts.CoreOpsDropDrop.drop (self : Inner) : RustM Inner := do
...
def OuterNoDrop.get_block_size (self : OuterNoDrop) : RustM Std.U32 := do
  ok self.block_size

/-- [drop_bug::{drop_bug::OuterNoDrop}::get_pad_value]:
    Source: 'tests/src/drop_bug.rs', lines 88:4-90:5 -/
def OuterNoDrop.get_pad_value (self : OuterNoDrop) : RustM Std.U8 := do
  ok self.pad_value

/-- [drop_bug::{drop_bug::OuterNoDrop}::init]:
    Source: 'tests/src/drop_bug.rs', lines 92:4-94:5 -/
def OuterNoDrop.init (self : OuterNoDrop) : RustM OuterNoDrop := do
