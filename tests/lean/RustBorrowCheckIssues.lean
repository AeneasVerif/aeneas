...
/-- [core::mem::drop]:
    Source: '/rustc/library/core/src/mem/mod.rs', lines 1045:0-1047:24
    Name pattern: [core::mem::drop]
    Visibility: public -/
@[rust_fun "core::mem::drop"] axiom core.mem.drop {T : Type} : T → RustM Unit
...
def unnecessary_error_2 (b0 : Bool) (b1 : Bool) : RustM Unit := do
  let i ← if b0
            then ok 0#u32
            else ok 1#u32
  let _ ←
...
def no_control_flow_loop.body
  ...
  RustM (ControlFlow ((Option X) × (Option X → Option X)) Unit)
  := do
  match p with
  | none => ok (done ())
...
def no_control_flow_loop
  (p : Option X) (back : Option X → Option X) : RustM Unit := do
  loop
...
@[reducible]
def no_control_flow : RustM Unit := do
...
def unwrap_option_mut
  {T : Type} (x : Option T) : RustM (T × (T → Option T)) := do
  match x with
  | none => fail panic
  | some x1 => ok (x1, some)

/-- [rust_borrow_check_issues::conditional_with_indirection]: loop body 0:
    Source: 'tests/src/rust-borrow-check-issues.rs', lines 92:4-96:5 -/
@[rust_loop_body]
def conditional_with_indirection_loop.body
  ...
  | some _ =>
    ...
    let (x, unwrap_option_mut_back) ← unwrap_option_mut o
    ok (cont (x.next,
      fun o1 =>
...
/-- [rust_borrow_check_issues::conditional_with_indirection]:
    Source: 'tests/src/rust-borrow-check-issues.rs', lines 89:0-97:1 -/
@[reducible]
def conditional_with_indirection : RustM Unit := do
...
def conditional_with_indirection_2_loop.body
  ...
  RustM (ControlFlow ((Option X) × (Option X → Option X) × Bool) Unit)
  := do
  match p with
  | none => ok (done ())
  | some _ =>
...
def conditional_with_indirection_2_loop
  (p : Option X) (back : Option X → Option X) (b0 : Bool) : RustM Unit := do
  loop
...
@[reducible]
def conditional_with_indirection_2 (b0 : Bool) : RustM Unit := do
