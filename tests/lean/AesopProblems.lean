...
JL: For `aesop_simp at ...`, backwards rules (`apply`, `constructors`, ...) don't
make sense, but we could still run safe `forward`/`destruct` rules.

SH: I think the backward rules still make sense: for instance, you could explore
...
JL: So I guess the `aesop_simp` algorithm should be:

- `aesop_simp` tries to simplify the target by
  - running all the `safe` rules until none apply any more;
  - clearing hypotheses introduced by these rules. (This is actually hard
    because Lean has trouble recognising which hypotheses did/didn't change. But
    I need to solve this problem anyway, so maybe now is the time to attack it.)
- `aesop_simp at h` tries to simplify the hypothesis `h` by
  - trying the `safe` rules which interact with `h`: `destruct`, `simp`,
    `unfold`, `cases` (what about `tactic` and `forward`?);
  - on the subgoals generated by these, run all `safe` tactics (or `unsafe`
    tactics as well?);
  - clearing hypotheses introduced by the rules in the second step (but not the
    first).

## Stability

JL: I'm also quite concerned about stability. The usual way we deal with this in
...
SH: Yes, stability will be an issue. But can we consider it is orthogonal for now?
...
JL: Indeed, this is something we can worry about later.
-/
...
JL: If you don't want to keep `U` (here: `i ≠ j`) around, then I think we should
not use the algorithm above but rather find a way to rewrite with the
conditional equation `index_update_ne : i ≠ j → index (update ...) ... = ...`
while letting Aesop solve the condition `i ≠ j`. The infrastructure for this
already exists in principle: `simp` can take a `discharger` tactic that is used
to discharge conditions.

The big question with this approach is performance. Naively, Aesop would try to
derive `i ≠ j` for each subgoal in which `index (update ...) ...` appears. This
seems terrible. Some ideas for controlling the search space:

- Severely reduce Aesop's search depth when it's used as a discharger.
- Use fancy caching to record the fact that we've already tried to prove `i ≠ j`
  and it didn't work. This has to be quite fancy, though, because the
  provability of `i ≠ j` depends on the entire context.
-/
...
JL: Aesop currently always performs case analysis on the condition of every
if-then-else in the goal, using the `split` tactic. So the if-then-else will get
reduced, but the resulting goals may not be so clear.

JL: A less hacky solution could be achieved with a variant of the new rule
...
SH: For the other alternative, I think we have to generate the goals:
  G₁ := Γ ⊢ i = j
...
JL: The implementation is very inefficient. Essentially, forward rules are not
cached at all, so they are applied at every goal where a hypothesis matches
a premise of the rule. This happens even if the rule has already been applied
to a parent goal, successfully or unsuccessfully. (If the rule already succeeded
on a parent goal, Aesop throws away its result for the child goal. :) I have
some ideas for how to make this stuff much less ridiculous, but the algorithm
is still a bit hazy.
-/
