...
structure To (Self : Type) where
  «to» : forall {T : Type} (FromInst : From T Self), Self → RustM T

/-- [from_to::{impl from_to::From<u32> for u32}::from]:
    Source: 'tests/src/from_to.rs', lines 14:4-16:5 -/
def U32.Insts.From_toFromU32.from (x : Std.U32) : RustM Std.U32 := do
  ok x
...
def U32.Insts.From_toTo.to
  {T : Type} (FromTU32Inst : From T Std.U32) (self : Std.U32) : RustM T := do
