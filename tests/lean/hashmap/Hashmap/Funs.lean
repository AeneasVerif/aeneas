...
def hash_map_clear_slots_loop_fwd_back
  ...
  then
    do
      ...
      let v ⟵ hash_map_clear_slots_loop_fwd_back T slots0 i1
      Result.ret v
  else Result.ret slots
termination_by hash_map_clear_slots_loop_fwd_back slots i =>
  hash_map_clear_slots_loop_terminates T slots i
decreasing_by hash_map_clear_slots_loop_decreases slots i

/- [hashmap::HashMap::{0}::clear_slots] -/
def hash_map_clear_slots_fwd_back
  (T : Type) (slots : Vec (list_t T)) : Result (Vec (list_t T)) :=
  do
    let v ⟵
      hash_map_clear_slots_loop_fwd_back T slots (Usize.ofInt 0 (by intlit))
    Result.ret v
...
def hash_map_clear_fwd_back
  ...
  do
    let v ⟵ hash_map_clear_slots_fwd_back T self.hash_map_slots
    Result.ret
      {
        self
          with
