...
def choose
  ...
  :=
  if b
  then let back_'a := fun ret => Result.ok (ret, y)
       Result.ok (x, back_'a)
  else let back_'a := fun ret => Result.ok (x, ret)
       Result.ok (y, back_'a)
...
divergent def list_nth_mut
  ...
  | List.Cons x tl =>
    if i = 0#u32
    then
      let back_'a := fun ret => Result.ok (List.Cons ret tl)
      Result.ok (x, back_'a)
    else
      do
      let i1 ← i - 1#u32
