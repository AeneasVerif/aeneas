-- THIS FILE WAS AUTOMATICALLY GENERATED BY AENEAS
-- [chunks_exact]
import Aeneas
open Aeneas Aeneas.Std RustM ControlFlow Error
set_option linter.dupNamespace false
set_option linter.hashCommand false
set_option linter.unusedVariables false

/- You can set the `maxHeartbeats` value with the `-max-heartbeats` CLI option -/
set_option maxHeartbeats 1000000

/- You can set the `maxRecDepth` value with the `-max-recdepth` CLI option -/
set_option maxRecDepth 2048

namespace chunks_exact

/-- [chunks_exact::test_chunks_exact_exact_fit]:
    Source: 'tests/src/chunks_exact.rs', lines 8:0-22:1
    Visibility: public -/
def test_chunks_exact_exact_fit : RustM Unit := do
  let s ←
    lift (Array.to_slice
      (Array.make 6#usize [ 1#u32, 2#u32, 3#u32, 4#u32, 5#u32, 6#u32 ]))
  let it ← core.slice.Slice.chunks_exact s 3#usize
  let (o, it1) ← core.slice.iter.IteratorChunksExact.next it
  let c1 ← core.option.Option.unwrap o
  let i ← Slice.index_usize c1 0#usize
  massert (i = 1#u32)
  let i1 ← Slice.index_usize c1 1#usize
  massert (i1 = 2#u32)
  let i2 ← Slice.index_usize c1 2#usize
  massert (i2 = 3#u32)
  let (o1, it2) ← core.slice.iter.IteratorChunksExact.next it1
  let c2 ← core.option.Option.unwrap o1
  let i3 ← Slice.index_usize c2 0#usize
  massert (i3 = 4#u32)
  let i4 ← Slice.index_usize c2 1#usize
  massert (i4 = 5#u32)
  let i5 ← Slice.index_usize c2 2#usize
  massert (i5 = 6#u32)
  let (o2, it3) ← core.slice.iter.IteratorChunksExact.next it2
  let b := core.option.Option.is_none o2
  massert b
  let rem ← core.slice.iter.ChunksExact.getRemainder it3
  let i6 := Slice.len rem
  massert (i6 = 0#usize)

/- Unit test for [chunks_exact::test_chunks_exact_exact_fit] -/
#assert (test_chunks_exact_exact_fit == ok ())

/-- [chunks_exact::test_chunks_exact_with_remainder]:
    Source: 'tests/src/chunks_exact.rs', lines 26:0-41:1
    Visibility: public -/
def test_chunks_exact_with_remainder : RustM Unit := do
  let s ←
    lift (Array.to_slice
      (Array.make 7#usize [ 1#u32, 2#u32, 3#u32, 4#u32, 5#u32, 6#u32, 7#u32 ]))
  let it ← core.slice.Slice.chunks_exact s 3#usize
  let (o, it1) ← core.slice.iter.IteratorChunksExact.next it
  let c1 ← core.option.Option.unwrap o
  let i ← Slice.index_usize c1 0#usize
  massert (i = 1#u32)
  let i1 ← Slice.index_usize c1 1#usize
  massert (i1 = 2#u32)
  let i2 ← Slice.index_usize c1 2#usize
  massert (i2 = 3#u32)
  let (o1, it2) ← core.slice.iter.IteratorChunksExact.next it1
  let c2 ← core.option.Option.unwrap o1
  let i3 ← Slice.index_usize c2 0#usize
  massert (i3 = 4#u32)
  let i4 ← Slice.index_usize c2 1#usize
  massert (i4 = 5#u32)
  let i5 ← Slice.index_usize c2 2#usize
  massert (i5 = 6#u32)
  let (o2, it3) ← core.slice.iter.IteratorChunksExact.next it2
  let b := core.option.Option.is_none o2
  massert b
  let rem ← core.slice.iter.ChunksExact.getRemainder it3
  let i6 := Slice.len rem
  massert (i6 = 1#usize)
  let i7 ← Slice.index_usize rem 0#usize
  massert (i7 = 7#u32)

/- Unit test for [chunks_exact::test_chunks_exact_with_remainder] -/
#assert (test_chunks_exact_with_remainder == ok ())

/-- [chunks_exact::test_chunks_exact_remainder_2]:
    Source: 'tests/src/chunks_exact.rs', lines 45:0-57:1
    Visibility: public -/
def test_chunks_exact_remainder_2 : RustM Unit := do
  let s ←
    lift (Array.to_slice
      (Array.make 5#usize [ 10#u32, 20#u32, 30#u32, 40#u32, 50#u32 ]))
  let it ← core.slice.Slice.chunks_exact s 3#usize
  let (o, it1) ← core.slice.iter.IteratorChunksExact.next it
  let c ← core.option.Option.unwrap o
  let i ← Slice.index_usize c 0#usize
  massert (i = 10#u32)
  let i1 ← Slice.index_usize c 1#usize
  massert (i1 = 20#u32)
  let i2 ← Slice.index_usize c 2#usize
  massert (i2 = 30#u32)
  let (o1, it2) ← core.slice.iter.IteratorChunksExact.next it1
  let b := core.option.Option.is_none o1
  massert b
  let rem ← core.slice.iter.ChunksExact.getRemainder it2
  let i3 := Slice.len rem
  massert (i3 = 2#usize)
  let i4 ← Slice.index_usize rem 0#usize
  massert (i4 = 40#u32)
  let i5 ← Slice.index_usize rem 1#usize
  massert (i5 = 50#u32)

/- Unit test for [chunks_exact::test_chunks_exact_remainder_2] -/
#assert (test_chunks_exact_remainder_2 == ok ())

/-- [chunks_exact::test_chunks_exact_size_1]:
    Source: 'tests/src/chunks_exact.rs', lines 61:0-73:1
    Visibility: public -/
def test_chunks_exact_size_1 : RustM Unit := do
  let s ←
    lift (Array.to_slice (Array.make 3#usize [ 10#u32, 20#u32, 30#u32 ]))
  let it ← core.slice.Slice.chunks_exact s 1#usize
  let (o, it1) ← core.slice.iter.IteratorChunksExact.next it
  let c1 ← core.option.Option.unwrap o
  let i ← Slice.index_usize c1 0#usize
  massert (i = 10#u32)
  let (o1, it2) ← core.slice.iter.IteratorChunksExact.next it1
  let c2 ← core.option.Option.unwrap o1
  let i1 ← Slice.index_usize c2 0#usize
  massert (i1 = 20#u32)
  let (o2, it3) ← core.slice.iter.IteratorChunksExact.next it2
  let c3 ← core.option.Option.unwrap o2
  let i2 ← Slice.index_usize c3 0#usize
  massert (i2 = 30#u32)
  let (o3, it4) ← core.slice.iter.IteratorChunksExact.next it3
  let b := core.option.Option.is_none o3
  massert b
  let rem ← core.slice.iter.ChunksExact.getRemainder it4
  let i3 := Slice.len rem
  massert (i3 = 0#usize)

/- Unit test for [chunks_exact::test_chunks_exact_size_1] -/
#assert (test_chunks_exact_size_1 == ok ())

/-- [chunks_exact::test_chunks_exact_empty]:
    Source: 'tests/src/chunks_exact.rs', lines 77:0-83:1
    Visibility: public -/
def test_chunks_exact_empty : RustM Unit := do
  let s ← lift (Array.to_slice (Std.Array.empty Std.U32))
  let it ← core.slice.Slice.chunks_exact s 3#usize
  let (o, it1) ← core.slice.iter.IteratorChunksExact.next it
  let b := core.option.Option.is_none o
  massert b
  let rem ← core.slice.iter.ChunksExact.getRemainder it1
  let i := Slice.len rem
  massert (i = 0#usize)

/- Unit test for [chunks_exact::test_chunks_exact_empty] -/
#assert (test_chunks_exact_empty == ok ())

/-- [chunks_exact::test_chunks_exact_chunk_larger_than_slice]:
    Source: 'tests/src/chunks_exact.rs', lines 87:0-95:1
    Visibility: public -/
def test_chunks_exact_chunk_larger_than_slice : RustM Unit := do
  let s ← lift (Array.to_slice (Array.make 2#usize [ 1#u32, 2#u32 ]))
  let it ← core.slice.Slice.chunks_exact s 5#usize
  let (o, it1) ← core.slice.iter.IteratorChunksExact.next it
  let b := core.option.Option.is_none o
  massert b
  let rem ← core.slice.iter.ChunksExact.getRemainder it1
  let i := Slice.len rem
  massert (i = 2#usize)
  let i1 ← Slice.index_usize rem 0#usize
  massert (i1 = 1#u32)
  let i2 ← Slice.index_usize rem 1#usize
  massert (i2 = 2#u32)

/- Unit test for [chunks_exact::test_chunks_exact_chunk_larger_than_slice] -/
#assert (test_chunks_exact_chunk_larger_than_slice == ok ())

/-- [chunks_exact::test_chunks_exact_chunk_equals_slice]:
    Source: 'tests/src/chunks_exact.rs', lines 99:0-109:1
    Visibility: public -/
def test_chunks_exact_chunk_equals_slice : RustM Unit := do
  let s ← lift (Array.to_slice (Array.make 3#usize [ 1#u32, 2#u32, 3#u32 ]))
  let it ← core.slice.Slice.chunks_exact s 3#usize
  let (o, it1) ← core.slice.iter.IteratorChunksExact.next it
  let c ← core.option.Option.unwrap o
  let i ← Slice.index_usize c 0#usize
  massert (i = 1#u32)
  let i1 ← Slice.index_usize c 1#usize
  massert (i1 = 2#u32)
  let i2 ← Slice.index_usize c 2#usize
  massert (i2 = 3#u32)
  let (o1, it2) ← core.slice.iter.IteratorChunksExact.next it1
  let b := core.option.Option.is_none o1
  massert b
  let rem ← core.slice.iter.ChunksExact.getRemainder it2
  let i3 := Slice.len rem
  massert (i3 = 0#usize)

/- Unit test for [chunks_exact::test_chunks_exact_chunk_equals_slice] -/
#assert (test_chunks_exact_chunk_equals_slice == ok ())

/-- [chunks_exact::test_chunks_exact_2_odd]:
    Source: 'tests/src/chunks_exact.rs', lines 113:0-126:1
    Visibility: public -/
def test_chunks_exact_2_odd : RustM Unit := do
  let s ←
    lift (Array.to_slice
      (Array.make 5#usize [ 1#u32, 2#u32, 3#u32, 4#u32, 5#u32 ]))
  let it ← core.slice.Slice.chunks_exact s 2#usize
  let (o, it1) ← core.slice.iter.IteratorChunksExact.next it
  let c1 ← core.option.Option.unwrap o
  let i ← Slice.index_usize c1 0#usize
  massert (i = 1#u32)
  let i1 ← Slice.index_usize c1 1#usize
  massert (i1 = 2#u32)
  let (o1, it2) ← core.slice.iter.IteratorChunksExact.next it1
  let c2 ← core.option.Option.unwrap o1
  let i2 ← Slice.index_usize c2 0#usize
  massert (i2 = 3#u32)
  let i3 ← Slice.index_usize c2 1#usize
  massert (i3 = 4#u32)
  let (o2, it3) ← core.slice.iter.IteratorChunksExact.next it2
  let b := core.option.Option.is_none o2
  massert b
  let rem ← core.slice.iter.ChunksExact.getRemainder it3
  let i4 := Slice.len rem
  massert (i4 = 1#usize)
  let i5 ← Slice.index_usize rem 0#usize
  massert (i5 = 5#u32)

/- Unit test for [chunks_exact::test_chunks_exact_2_odd] -/
#assert (test_chunks_exact_2_odd == ok ())

/-- [chunks_exact::test_chunks_exact_2_single_element]:
    Source: 'tests/src/chunks_exact.rs', lines 130:0-137:1
    Visibility: public -/
def test_chunks_exact_2_single_element : RustM Unit := do
  let s ← lift (Array.to_slice (Array.make 1#usize [ 42#u32 ]))
  let it ← core.slice.Slice.chunks_exact s 2#usize
  let (o, it1) ← core.slice.iter.IteratorChunksExact.next it
  let b := core.option.Option.is_none o
  massert b
  let rem ← core.slice.iter.ChunksExact.getRemainder it1
  let i := Slice.len rem
  massert (i = 1#usize)
  let i1 ← Slice.index_usize rem 0#usize
  massert (i1 = 42#u32)

/- Unit test for [chunks_exact::test_chunks_exact_2_single_element] -/
#assert (test_chunks_exact_2_single_element == ok ())

end chunks_exact
