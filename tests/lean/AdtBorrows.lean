...
def SharedWrapper2.create
  {T : Type} (x : T) (y : T) : RustM (SharedWrapper2 T) := do
  ok { x, y }
...
def SharedWrapper2.unwrap
  {T : Type} (self : SharedWrapper2 T) : RustM (T × T) := do
  ok (self.x, self.y)

/-- [adt_borrows::use_shared_wrapper2]:
    Source: 'tests/src/adt-borrows.rs', lines 62:0-69:1 -/
def use_shared_wrapper2 : RustM Unit := do
...
def MutWrapper.id
  ...
  RustM ((MutWrapper T) × (MutWrapper T → MutWrapper T))
  := do
  let back := fun mw => mw
  ok (self, back)
...
def use_mut_wrapper : RustM Unit := do
...
def MutWrapper1.create
  {T : Type} (x : T) : RustM ((MutWrapper1 T) × (MutWrapper1 T → T)) := do
...
def MutWrapper1.unwrap
  {T : Type} (self : MutWrapper1 T) : RustM (T × (T → MutWrapper1 T)) := do
...
def MutWrapper1.id
  ...
  RustM ((MutWrapper1 T) × (MutWrapper1 T → MutWrapper1 T))
  := do
  ok (self, fun mw => mw)

/-- [adt_borrows::use_mut_wrapper1]:
    Source: 'tests/src/adt-borrows.rs', lines 117:0-123:1 -/
def use_mut_wrapper1 : RustM Unit := do
...
def MutWrapper2.create
  {T : Type} (x : T) (y : T) :
  RustM ((MutWrapper2 T) × (MutWrapper2 T → T) × (MutWrapper2 T → T))
  := do
...
def MutWrapper2.unwrap
  ...
  RustM ((T × T) × (T → MutWrapper2 T) × (T → MutWrapper2 T))
  := do
...
def MutWrapper2.id
  {T : Type} (self : MutWrapper2 T) :
  RustM ((MutWrapper2 T) × (MutWrapper2 T → MutWrapper2 T) × (MutWrapper2 T
    → MutWrapper2 T))
  := do
...
def use_mut_wrapper2 : RustM Unit := do
...
def array_shared_borrow
  {N : Std.Usize} (x : Array Std.U32 N) : RustM (Array Std.U32 N) := do
  ok x
...
def array_mut_borrow
  ...
  RustM ((Array Std.U32 N) × (Array Std.U32 N → Array Std.U32 N))
  := do
  ok (x, fun x1 => x1)
...
def boxed_slice_shared_borrow (x : Slice Std.U32) : RustM (Slice Std.U32) := do
  ok x
...
/-- [adt_borrows::{adt_borrows::SharedList<'a, T>}::push]:
    ...
    Visibility: public -/
def SharedList.push
  {T : Type} (self : SharedList T) (x : T) : RustM (SharedList T) := do
...
/-- [adt_borrows::{adt_borrows::SharedList<'a, T>}::pop]:
    ...
    Visibility: public -/
def SharedList.pop
  {T : Type} (self : SharedList T) : RustM (T × (SharedList T)) := do
  match self with
...
def MutList.push
  ...
  RustM ((MutList T) × (MutList T → ((MutList T) × T)))
  := do
  let back :=
    fun ml =>
      let (x1, ml1) :=
...
def MutList.pop
  ...
  RustM ((T × (MutList T)) × ((T × (MutList T)) → MutList T))
  := do
  match self with
...
/-- [adt_borrows::wrap_shared_in_option]:
    ...
    Visibility: public -/
def wrap_shared_in_option {T : Type} (x : T) : RustM (Option T) := do
  ok (some x)
...
def wrap_mut_in_option
  {T : Type} (x : T) : RustM ((Option T) × (Option T → T)) := do
