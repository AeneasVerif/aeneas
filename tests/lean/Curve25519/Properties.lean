...
def Scalar52.asNat (limbs : Array U64 5#usize) : Nat :=
  ∑ i : Fin 5, 2^(52 * i.val) * (limbs.val[i.val]).val
...
def Scalar52.wideAsNat (limbs : Array U128 9#usize) : Nat :=
  ∑ i : Fin 9, 2^(52 * i.val) * (limbs.val[i.val]).val
...
theorem mul_internal_spec (a b : Array U64 5#usize)
    (ha : ∀ (i : Nat) (hi : i < 5), (a.val[i]).val < 2 ^ 62)
    (hb : ∀ (i : Nat) (hi : i < 5), (b.val[i]).val < 2 ^ 62) :
  ...
  step*
  -- Post-condition
  simp [*, Scalar52.wideAsNat, Scalar52.asNat, Finset.sum_fin_eq_sum_range, Finset.sum_range_succ]
  ring_nf
