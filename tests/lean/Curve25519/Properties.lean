...
def Scalar52.asNat (limbs : Array U64 5#usize) : Nat :=
  ∑ i ∈ Finset.range 5, 2^(52 * i) * (limbs[i]!).toNat
...
def Scalar52.wideAsNat (limbs : Array U128 9#usize) : Nat :=
  ∑ i ∈ Finset.range 9, 2^(52 * i) * (limbs[i]!).toNat

attribute [-simp] Int.reducePow Nat.reducePow

@[step]
theorem m_spec (x y : U64) :
    m x y ⦃ result => result.toNat = x.toNat * y.toNat ⦄ := by
  unfold m
  step*
...
theorem mul_internal_spec (a b : Array U64 5#usize)
    (ha : ∀ i < 5, a[i]!.toNat < 2 ^ 62) (hb : ∀ i < 5, b[i]!.toNat < 2 ^ 62) :
