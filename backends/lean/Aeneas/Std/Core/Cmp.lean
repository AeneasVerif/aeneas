...
structure core.cmp.PartialEq (Self : Type) (Rhs : Type) where
  ...
  ne : Self → Rhs → RustM Bool := fun self other => do ok (not (← eq self other))
...
structure core.cmp.Eq (Self : Type) where
  ...
  assert_fields_are_eq (_ : Self) : RustM Unit := .ok ()

@[simp, rust_fun "core::cmp::Eq::assert_fields_are_eq"]
def core.cmp.Eq.assert_fields_are_eq.default
  {Self : Type} (EqInst : core.cmp.Eq Self) (x : Self) : RustM Unit :=
  EqInst.assert_fields_are_eq x

/- Default method. -/
def core.cmp.PartialEq.ne.default {Self Rhs : Type} (eq : Self → Rhs → RustM Bool)
  (self : Self) (other : Rhs) : RustM Bool := do
  ok (¬ (← eq self other))
...
def core.cmp.PartialEq.ne.trait_default {Self Rhs : Type}
  ...
  (self : Self) (other : Rhs) : RustM Bool :=
...
theorem core.cmp.PartialEq.ne.default.spec {T : Type}
  (eq : T → T → RustM Bool) (self : T) (other : T)
...
def core.cmp.PartialOrd.lt_body {Self Rhs : Type}
  (partial_cmp : Self → Rhs → RustM (Option Ordering))
...
structure core.cmp.PartialOrd (Self : Type) (Rhs : Type) where
  partialEqInst : core.cmp.PartialEq Self Rhs
  partial_cmp : Self → Rhs → RustM (Option Ordering)
  lt : Self → Rhs → RustM Bool := core.cmp.PartialOrd.lt_body partial_cmp
  le : Self → Rhs → RustM Bool := core.cmp.PartialOrd.le_body partial_cmp
  gt : Self → Rhs → RustM Bool := core.cmp.PartialOrd.gt_body partial_cmp
  ge : Self → Rhs → RustM Bool := core.cmp.PartialOrd.ge_body partial_cmp

/- Default method -/
@[rust_fun "core::cmp::PartialOrd::lt"]
def core.cmp.PartialOrd.lt.default {Self Rhs : Type}
  (partial_cmp : Self → Rhs → RustM (Option Ordering))
  (x : Self) (y : Rhs) : RustM Bool :=
...
structure core.cmp.Ord (Self : Type) where
  ...
  cmp : Self → Self → RustM Ordering
  max : Self → Self → RustM Self :=
    core.cmp.Ord.max_body partialOrdInst.lt
  min : Self → Self → RustM Self :=
    core.cmp.Ord.min_body partialOrdInst.lt
  clamp : Self → Self → Self → RustM Self :=
    core.cmp.Ord.clamp_body partialOrdInst.le partialOrdInst.lt partialOrdInst.gt

/- Default method -/
@[rust_fun "core::cmp::Ord::max"]
def core.cmp.Ord.max.default {Self : Type} (lt : Self → Self → RustM Bool)
  (x y : Self) : RustM Self :=
  core.cmp.Ord.max_body lt x y

def core.cmp.Ord.min.default {Self : Type} (lt : Self → Self → RustM Bool)
  (x y : Self) : RustM Self :=
...
def core.cmp.Ord.clamp.default {Self : Type} (le lt gt : Self → Self → RustM Bool)
  (self min max : Self) : RustM Self :=
  core.cmp.Ord.clamp_body le lt gt self min max

@[simp, rust_fun "core::cmp::min"]
def core.cmp.min {T : Type} (OrdInst : core.cmp.Ord T) (x y : T) : RustM T :=
  -- TODO: is this the correct model?
  OrdInst.min x y

@[simp, rust_fun "core::cmp::max"]
def core.cmp.max {T : Type} (OrdInst : core.cmp.Ord T) (x y : T) : RustM T :=
  -- TODO: is this the correct model?
  OrdInst.max x y

@[simp, rust_fun "core::cmp::impls::{core::cmp::PartialEq<(), ()>}::eq"]
def core.cmp.impls.PartialEqUnit.eq (_ _ : Unit) : RustM Bool := ok true

@[simp, rust_fun "core::cmp::impls::{core::cmp::PartialEq<(), ()>}::ne"]
def core.cmp.impls.PartialEqUnit.ne (_ _ : Unit) : RustM Bool := ok false
...
def core.cmp.impls.PartialOrdUnit.partial_cmp (_ _ : Unit) : RustM (Option Ordering) :=
  ok (some .eq)

@[simp, rust_fun "core::cmp::impls::{core::cmp::Ord<()>}::cmp"]
def core.cmp.impls.OrdUnit.cmp (_ _ : Unit) : RustM Ordering :=
  ok .eq

@[rust_fun "core::cmp::impls::{core::cmp::PartialEq<bool, bool>}::eq"]
def core.cmp.impls.PartialEqBool.eq (b0 b1 : Bool) : RustM Bool := .ok (b0 = b1)
...
def alloc.boxed.PartialEqBox.eq
  {T : Type} (PartialEqInst : core.cmp.PartialEq T T) (x y : T) : RustM Bool :=
  PartialEqInst.eq x y
