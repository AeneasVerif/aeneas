...
def alloc.vec.into_iter.IteratorIntoIter.next {T : Type} (it: alloc.vec.into_iter.IntoIter T) :
  RustM ((Option T) × (alloc.vec.into_iter.IntoIter T)) :=
  match it with
  | ⟨ [], _ ⟩  => ok (none, it)
...
@[rust_fun
  ...
  (keepParams := [true, false])]
def alloc.vec.IntoIteratorVec.into_iter {T : Type} (v: alloc.vec.Vec T) : RustM (alloc.vec.into_iter.IntoIter T) := ok v
...
def alloc.vec.FromIteratorVec.iterToList
    ...
    (iter : IntoIter) (acc : List T) : RustM (List T) := do
...
def alloc.vec.into_iter.IntoIter.Insts.CoreIterTraitsIteratorIterator.map
  ...
  RustM (core.iter.adapters.map.Map (alloc.vec.into_iter.IntoIter T) F) :=
  fun it f => .ok ⟨ it, f ⟩
