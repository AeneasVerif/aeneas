...
theorem Vec.getElem?_Usize_eq {α : Type u} (v : Vec α) (i : Usize) : v[i]? = v.val[i.val]? := by rfl

@[simp, scalar_tac_simps, simp_lists_safe, simp_lists_hyps_simps, grind =, agrind =]
theorem Vec.getElem!_Usize_eq {α : Type u} [Inhabited α] (v : Vec α) (i : Usize) : v[i]! = v.val[i.val]! := by rfl
...
def core.slice.index.SliceIndexRangeToUsizeVec.get
  {T : Type} (r : core.ops.range.RangeTo Usize) (s : Vec T) : Result (Option (Vec T)) :=
...
def core.slice.index.SliceIndexRangeToUsizeVec.get_mut
  {T : Type} (r : core.ops.range.RangeTo Usize) (s : Vec T) :
  Result ((Option (Vec T)) × (Option (Vec T) → Vec T)) :=
  if r.end ≤ s.length then
    ok (some (.from (s.val.slice 0 r.end) (by scalar_tac)),
        fun s' =>
        match s' with
...
def core.slice.index.SliceIndexRangeToUsizeVec.get_unchecked
  {T : Type} (_ : core.ops.range.RangeTo Usize) (_ : ConstRawPtr (Vec T)) : Result (ConstRawPtr (Vec T)) :=
...
def core.slice.index.SliceIndexRangeToUsizeVec.get_unchecked_mut
  {T : Type} (_ : core.ops.range.RangeTo Usize) (_ : MutRawPtr (Vec T)) :
  Result (MutRawPtr (Vec T)) :=
  -- TODO: update once we make the model of computation more stateful (for now we just fail)
  fail .undef

@[rust_fun "core::slice::index::{core::slice::index::SliceIndex<core::ops::range::RangeTo<usize>, [@T], [@T]>}::index"]
def core.slice.index.SliceIndexRangeToUsizeVec.index
  {T : Type} (r : core.ops.range.RangeTo Usize) (s : Vec T) : Result (Vec T) :=
  if r.end ≤ s.length then
...
def core.slice.index.SliceIndexRangeToUsizeVec.index_mut
  {T : Type} (r : core.ops.range.RangeTo Usize) (s : Vec T) :
...
def core.slice.index.SliceIndexRangeToUsizeVec (T : Type) :
  ...
  get := core.slice.index.SliceIndexRangeToUsizeVec.get
  get_mut := core.slice.index.SliceIndexRangeToUsizeVec.get_mut
  get_unchecked := core.slice.index.SliceIndexRangeToUsizeVec.get_unchecked
  get_unchecked_mut := core.slice.index.SliceIndexRangeToUsizeVec.get_unchecked_mut
  index := core.slice.index.SliceIndexRangeToUsizeVec.index
  index_mut := core.slice.index.SliceIndexRangeToUsizeVec.index_mut
}
...
theorem Vec.index_RangeTo_spec {α : Type} (v : Vec α) (r : core.ops.range.RangeTo Usize)
    (h : r.end ≤ v.length) :
    Vec.index (core.slice.index.SliceIndexRangeToUsizeVec α) v r
    ⦃ s1 =>
...
@[step]
theorem core.slice.index.SliceIndexRangeToUsizeVec.index.step_spec
    (r : core.ops.range.RangeTo Usize) (s : Vec α) (h : r.end ≤ s.length) :
  core.slice.index.SliceIndexRangeToUsizeVec.index r s
    ⦃ s1 =>
...
theorem Vec.index_mut_RangeTo_spec {α : Type} (v : Vec α) (r : core.ops.range.RangeTo Usize)
    (h : r.end ≤ v.length) :
    Vec.index_mut (core.slice.index.SliceIndexRangeToUsizeVec α) v r
    ⦃ (s1 : Vec α) (back : Vec α → Vec α) =>
      s1.val = v.val.slice 0 r.end ∧
      s1.length = r.«end» ∧
