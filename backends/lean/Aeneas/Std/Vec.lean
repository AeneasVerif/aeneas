...
theorem alloc.vec.from_elem_spec {T : Type} (cloneInst : core.clone.Clone T)
  ...
  v.length = n.val ⦄ := by
  unfold from_elem
  obtain ⟨ l, hcl, hval, _ ⟩ := spec_imp_exists (@List.clone_spec _ cloneInst.clone (List.replicate n.val x) (by intros; simp_all))
  rw [hcl]
  refine ⟨hval, ?_⟩
  simp only [Vec.length, hval, List.length_replicate]
...
theorem alloc.vec.Vec.set_getElem!_eq α [Inhabited α] (x : alloc.vec.Vec α) (i : Usize) :
  x.set i x[i]! = x := by
  apply Subtype.ext
  simp only [set_val_eq, getElem!_Usize_eq]
  simp_lists
