...
-- Conversions
@[expose] def FromUsizeBool.from (b : Bool) : Usize :=
  if b then 1#usize else 0#usize

@[expose] def FromU8Bool.from (b : Bool) : U8 :=
  if b then 1#u8 else 0#u8

@[expose] def FromU16Bool.from (b : Bool) : U16 :=
  if b then 1#u16 else 0#u16

@[expose] def FromU32Bool.from (b : Bool) : U32 :=
  if b then 1#u32 else 0#u32

@[expose] def FromU64Bool.from (b : Bool) : U64 :=
  if b then 1#u64 else 0#u64

@[expose] def FromU128Bool.from (b : Bool) : U128 :=
  if b then 1#u128 else 0#u128

@[expose] def FromIsizeBool.from (b : Bool) : Isize :=
  if b then 1#isize else 0#isize

@[expose] def FromI8Bool.from (b : Bool) : I8 :=
  if b then 1#i8 else 0#i8

@[expose] def FromI16Bool.from (b : Bool) : I16 :=
  if b then 1#i16 else 0#i16

@[expose] def FromI32Bool.from (b : Bool) : I32 :=
  if b then 1#i32 else 0#i32

@[expose] def FromI64Bool.from (b : Bool) : I64 :=
  if b then 1#i64 else 0#i64

@[expose] def FromI128Bool.from (b : Bool) : I128 :=
  if b then 1#i128 else 0#i128

@[expose] def FromUsizeU8.from (x : U8) : Usize := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromUsizeU16.from (x : U16) : Usize := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromUsizeU32.from (x : U32) : Usize := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromUsizeUsize.from (x : Usize) : Usize := ⟨ x.bv.setWidth _ ⟩

@[expose] def FromU8U8.from (x : U8) : U8 := ⟨ x.bv.setWidth _ ⟩

@[expose] def FromU16U8.from (x : U8) : U16 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU16U16.from (x : U16) : U16 := ⟨ x.bv.setWidth _ ⟩

@[expose] def FromU32U8.from (x : U8) : U32 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU32U16.from (x : U16) : U32 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU32U32.from (x : U32) : U32 := ⟨ x.bv.setWidth _ ⟩

@[expose] def FromU64U8.from (x : U8) : U64 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU64U16.from (x : U16) : U64 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU64U32.from (x : U32) : U64 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU64U64.from (x : U64) : U64 := ⟨ x.bv.setWidth _ ⟩

@[expose] def FromU128U8.from (x : U8) : U128 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU128U16.from (x : U16) : U128 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU128U32.from (x : U32) : U128 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU128U64.from (x : U64) : U128 := ⟨ x.bv.setWidth _ ⟩
@[expose] def FromU128U128.from (x : U128) : U128 := ⟨ x.bv.setWidth _ ⟩

@[expose] def FromIsizeI8.from (x : I8) : Isize := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromIsizeI16.from (x : I16) : Isize := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromIsizeI32.from (x : I32) : Isize := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromIsizeIsize.from (x : Isize) : Isize := ⟨ x.bv.signExtend _ ⟩

@[expose] def FromI8I8.from (x : I8) : I8 := ⟨ x.bv.signExtend _ ⟩

@[expose] def FromI16I8.from (x : I8) : I16 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI16I16.from (x : I16) : I16 := ⟨ x.bv.signExtend _ ⟩

@[expose] def FromI32I8.from (x : I8) : I32 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI32I16.from (x : I16) : I32 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI32I32.from (x : I32) : I32 := ⟨ x.bv.signExtend _ ⟩

@[expose] def FromI64I8.from (x : I8) : I64 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI64I16.from (x : I16) : I64 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI64I32.from (x : I32) : I64 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI64I64.from (x : I64) : I64 := ⟨ x.bv.signExtend _ ⟩

@[expose] def FromI128I8.from (x : I8) : I128 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI128I16.from (x : I16) : I128 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI128I32.from (x : I32) : I128 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI128I64.from (x : I64) : I128 := ⟨ x.bv.signExtend _ ⟩
@[expose] def FromI128I128.from (x : I128) : I128 := ⟨ x.bv.signExtend _ ⟩
...
iscalar_no_isize def core.num.«%S».to_le_bytes (x : «%S») : Array I8 (%Size)#usize :=
  .from (x.bv.toLEBytes.map IScalar.mk) (by simp [BitVec.toLEBytes_length])
...
iscalar_no_isize def core.num.«%S».to_be_bytes (x : «%S») : Array I8 (%Size)#usize :=
  .from (x.bv.toBEBytes.map IScalar.mk) (by simp [BitVec.toBEBytes_length])
...
theorem core.num.«%S».to_le_bytes.step_spec (x : «%S») :
  ...
  simp only [spec_ok, lift, Array.from_val, to_le_bytes, UScalarTy.U8_numBits_eq]

iscalar_no_isize @[step]
theorem core.num.«%S».to_le_bytes.step_spec (x : «%S») :
  lift (core.num.«%S».to_le_bytes x) ⦃ y => y.val = x.bv.toLEBytes.map (@IScalar.mk IScalarTy.I8) ⦄ := by
  simp only [spec_ok, lift, Array.from_val, to_le_bytes, IScalarTy.I8_numBits_eq]
...
theorem core.num.«%S».to_be_bytes.step_spec (x : «%S») :
  ...
  simp only [spec_ok, lift, Array.from_val, to_be_bytes, UScalarTy.U8_numBits_eq]

iscalar_no_isize @[step]
theorem core.num.«%S».to_be_bytes.step_spec (x : «%S») :
  lift (core.num.«%S».to_be_bytes x) ⦃ y => y.val = x.bv.toBEBytes.map (@IScalar.mk IScalarTy.I8) ⦄ := by
  simp only [spec_ok, lift, Array.from_val, to_be_bytes, IScalarTy.I8_numBits_eq]
...
@[expose, reducible, rust_type "core::num::error::TryFromIntError"]
def core.num.error.TryFromIntError : Type := Unit

/-- General fallible conversion between unsigned scalar types. -/
@[expose] def core.num.tryFromUScalar {srcTy : UScalarTy} (tgtTy : UScalarTy) (i : UScalar srcTy) :
...
@[expose, rust_fun "core::convert::num::ptr_try_from_impls::{core::convert::TryFrom<u32, usize, core::num::error::TryFromIntError>}::try_from"]
