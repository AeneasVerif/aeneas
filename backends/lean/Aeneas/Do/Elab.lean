...
partial def mkUncurries (innerLam : Expr) (types : List Expr) : MetaM Expr := do
  match types with
  | [] | [_] => return innerLam
  | [_, _] => mkAppM ``_root_.Aeneas.Std.WP.uncurry #[innerLam]
  | _ :: rest =>
    lambdaBoundedTelescope innerLam 1 fun fvars body => do
      ...
      mkAppM ``_root_.Aeneas.Std.WP.uncurry #[newLam]
...
/-- First non-anonymous leaf name in the pattern, if any. Used to give
    ...
    name survives `WP.uncurry`-reduction in `step*` analysis. -/
...
/-- Unpack `fv : ty` per `sub`, calling `body` with the leaves. Emits
    `WP.uncurry` for `.prod` and `T.casesOn` for `.ctor`. -/
...
partial def elabDoLetPat (pat : Term) (rhs : Term)
  ...
  -- For `.prod` the head is `WP.uncurry`, so `headBeta` is a no-op.
