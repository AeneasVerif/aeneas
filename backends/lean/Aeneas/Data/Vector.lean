...
@[simp_lists_safe]
theorem getElem_cast' {n m : ℕ} {α : Type u_1} [Inhabited α] (i : ℕ) (h: n = m ∧ i < n)
  (v : Vector α n) :
  (Vector.cast h.1 v)[i] = v[i] := by
...
theorem eq_iff_forall_eq_getElem {α} {n} [Inhabited α] (l0 l1 : Vector α n) :
  l0 = l1 ↔ (∀ i, (_ : i < n) → l0[i] = l1[i]) := by
  grind

@[simp_lists_safe]
theorem getElem!_map_eq {α β} [Inhabited α] [Inhabited β] {n} (f : α → β)
  (x : Vector α n) (i : ℕ) (hi : i < n) :
  (x.map f)[i]! = f x[i]! := by
...
@[simp_lists_safe]
theorem getElem_map_eq {α β} [Inhabited α] [Inhabited β] {n} (f : α → β)
  (x : Vector α n) (i : ℕ) (hi : i < n) :
  (x.map f)[i] = f x[i] := by
  grind
...
@[simp_lists_safe]
theorem getElem_range_of_lt {n i : Nat}  (hi : i < n) : (Vector.range n)[i] = i := by
  grind
...
attribute [simp_lists] getElem_ofFn getElem_toArray
