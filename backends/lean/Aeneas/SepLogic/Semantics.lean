...
events, and `Aeneas.Std.WP` builds the correctness judgments and the ispecs
over it *denotationally*.  This file supplies the operational counterpart, in
three layers:

* the **runs** of that machine: `Aeneas.Std.WP` gives heap events their
...
answered on one heap — is defined in `Aeneas.Std.WP`, where the correctness
...
Both halves are the generic adequacy proved in `Aeneas.Data.Coinductive.StateMachine` —
