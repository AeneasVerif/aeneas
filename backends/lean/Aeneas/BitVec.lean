...
theorem Aeneas.BitVec.ofNat_pow {n : ℕ} (x d : ℕ) : BitVec.ofNat n (x ^ d) = (BitVec.ofNat n x)^d := by
  revert x
  induction d <;> simp_all [pow_succ]

@[simp]
theorem Aeneas.BitVec.toNat_pow {w : ℕ} (x : BitVec w) (d : ℕ) :
...
theorem BitVec.twoPow_eq_two_pow {w i} : BitVec.twoPow w i = 2#w ^ i := by
  natify; simp only [Aeneas.BitVec.toNat_pow, toNat_ofNat]
...
theorem BitVec.and_two_pow_sub_one_eq_mod {w} (x : BitVec w) (n : Nat) :
  ...
  by_cases hn : n < w
  . simp only [← Aeneas.BitVec.ofNat_pow]
    natify
    simp
    have : 2^n < 2^w := by
...
theorem BitVec.shiftLeft_sub_one_eq_mod {w} (x : BitVec w) (n : Nat) :
  ...
  have : 1#w * BitVec.twoPow w n = 2#w ^ n := by
    ...
    rw [this]
    ring_nf
    natify; simp only [Aeneas.BitVec.toNat_pow, BitVec.toNat_ofNat]
    zmodify
  rw [this]
