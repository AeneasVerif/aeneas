...
| [`PureSpec.lean`](Tests/PureSpec.lean) | Regression tests for the pure-computation notation: that `m ⦃ x => p ⦄` and `⦃ emp ⦄ m ⦃⇓ x => ⌜p⌝ ⦄` are the *same* proposition, the round trip through the delaborator, the interoperability one judgment buys — a pure specification framed into a heap proof, a heap proof behind a pure specification, both under one `step*`, and higher-order contracts in either form — and what the notation gives up, recovered by `triple_emp_eq_ok`. |
