...
inductive RustEffect.I : Type 1 where
-- we represent partially defined stateful operations as guarded operations
-- this representations avoids the alternative cases: a lot of syntax, or requiring decidable equality for types
| guardedModify (α : Type) (pre : Heap → Prop) (modify : (h : Heap) → pre h → α × Heap) : RustEffect.I
| fail : Error → RustEffect.I

def RustEffect.O (i : RustEffect.I) : Type 1 :=
  match i with
