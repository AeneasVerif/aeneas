...
axiom solvingTriple_pull {P : Prop} {m : TestM α} {Q : Post α} :
    solvingTriple P m Q ↔ (P → solvingTriple True m Q)

axiom solvingTriple_step_mono {Pm : Prop} {Q : Post α}
    (m : TestM α) (Qm : Post α) (hStep : solvingTriple Pm m Qm)
    (hPre : True → Pm)
    (hPost : Post.entails Qm Q) :
    solvingTriple True m Q

axiom introGoal : Prop
axiom introGoal_proof : introGoal

axiom solvingTriple_step_bind {Pm : Prop} {next : α → TestM β} {Q : Post β}
    (m : TestM α) (Qm : Post α) (hStep : solvingTriple Pm m Qm)
    (hPre : True → Pm)
    (hNext : ∀ _value : α, introGoal) :
    solvingTriple True (m >>= next) Q

syntax "solve_intro" : tactic

macro_rules
  | `(tactic| solve_intro) =>
    `(tactic| first | rw [solvingTriple_pull] | exact introGoal_proof)
...
#register_spec_info {
    ...
    qimp_elim_tactics := #[``Post.entails_iff, ``true_imp_iff]
    intro_tactic := SpecInfo.tac `(tactic| solve_intro)
    to_mvcgen := none
    liftings := #[]
  }
...
/- `intro_tactic` is not applied to the initial goal, so `step` cannot match
the `solvingTriple True` step rule. Running it explicitly extracts `introGoal`
from the triple; after that, `step` applies it to the bind continuation and
closes the goal. -/
example (value : Nat) :
    solvingTriple introGoal (incrTwice value) (fun result => result = value + 2) := by
  unfold incrTwice
  fail_if_success
    step
  solve_intro
  intro introGoal_proof
  step
