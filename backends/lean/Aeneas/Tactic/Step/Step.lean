...
def getBindArgs? (program : Expr) : Option (Expr × Expr) :=
...
def getCallSiteTree (info : SpecInfo) (isLet : Bool) (goal : MVarId) :
    MetaM NameTree := do
...
-/
def introOutputs (info : SpecInfo) (args : Args) (fExpr : Expr) (callSiteTree : NameTree)
    (stepState : StepState) :
