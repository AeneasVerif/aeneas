...
attribute [simp_scalar_safe] ge_iff_le gt_iff_lt

@[simp_scalar_safe] theorem Nat.eq_imp_eq_true (x y : ℕ) (h : x = y) : (x = y) ↔ True := by simp [*]
@[simp_scalar_safe] theorem Nat.ne_imp_ne_true (x y : ℕ) (h : ¬ x = y) : (¬ x = y) ↔ True := by simp [*]
@[simp_scalar_safe] theorem Int.eq_imp_eq_true (x y : ℤ) (h : x = y) : (x = y) ↔ True := by simp [*]
@[simp_scalar_safe] theorem Int.ne_imp_ne_true (x y : ℤ) (h : ¬ x = y) : (¬ x = y) ↔ True := by simp [*]

@[simp_scalar_safe] theorem Nat.ne_imp_eq_false (x y : ℕ) (h : ¬ x = y) : (x = y) ↔ False := by simp [*]
@[simp_scalar_safe] theorem Nat.eq_imp_ne_false (x y : ℕ) (h : x = y) : (¬ x = y) ↔ False := by simp [*]
@[simp_scalar_safe] theorem Int.ne_imp_eq_false (x y : ℤ) (h : ¬ x = y) : (x = y) ↔ False := by simp [*]
@[simp_scalar_safe] theorem Int.eq_imp_ne_false (x y : ℤ) (h : x = y) : (¬ x = y) ↔ False := by simp [*]

@[simp_scalar_safe] theorem Nat.lt_imp_lt (a b : ℕ) : a < b → a < b := by omega
@[simp_scalar_safe] theorem Nat.le_imp_le (a b : ℕ) : a ≤ b → a ≤ b := by omega

@[simp_scalar_safe] theorem Int.lt_imp_lt (a b : ℤ) : a < b → a < b := by omega
@[simp_scalar_safe] theorem Int.le_imp_le (a b : ℤ) : a ≤ b → a ≤ b := by omega

@[simp_scalar_safe] theorem Nat.ge_imp_lt_iff_false (a b : ℕ) : a ≥ b → (a < b ↔ False) := by
  simp +contextual only [ge_iff_le, iff_false, not_lt, implies_true]

@[simp_scalar_safe] theorem Nat.gt_imp_le_iff_false (a b : ℕ) : a > b → (a ≤ b ↔ False) := by
  simp +contextual only [gt_iff_lt, iff_false, not_le, implies_true]

@[simp_scalar_safe] theorem Int.ge_imp_lt_iff_false (a b : ℤ) : a ≥ b → (a < b ↔ False) := by
  simp +contextual only [ge_iff_le, iff_false, not_lt, implies_true]

@[simp_scalar_safe] theorem Int.gt_imp_le_iff_false (a b : ℤ) : a > b → (a ≤ b ↔ False) := by
  simp +contextual only [gt_iff_lt, iff_false, not_le, implies_true]

attribute [simp_scalar_safe] Nat.mod_eq_of_lt
...
attribute [simp_scalar_safe] Nat.div_le_self Nat.shiftRight_le
...
attribute [simp_scalar_safe] Nat.sub_eq_zero_of_le
...
@[simp_scalar_safe, simp_lists_safe]
...
def simpScalarTac (config : ScalarTac.CondSimpTacConfig)
  ...
  let mut simpThms := #[← simpScalarSafeSimpExt.getTheorems, ← SimpBoolProp.simpBoolPropSimpExt.getTheorems]
  let mut simprocs := #[← simpScalarSafeSimprocExt.getSimprocs, ← SimpBoolProp.simpBoolPropSimprocExt.getSimprocs]
  if !config.safe then
    simpThms := simpThms.push (← simpScalarSimpExt.getTheorems)
    simprocs := simprocs.push (← simpScalarSimprocExt.getSimprocs)
...
It is essentially equivalent to `simp (discharger := scalar_tac) [simp_bool_prop, simp_scalar_safe, simp_scalar]`
...
You can mark lemmas to be used by `simp_scalar` by annotating them with:
- `@[simp_scalar_safe]` for safe lemmas (always used, even with `simp_scalar +safe`)
- `@[simp_scalar]` for general lemmas (only used when `safe` is `false`, which is the default)

The `+safe` option restricts `simp_scalar` to only the safe lemmas.

# Marking lemmas with `simp_scalar_safe`

When marking lemmas with `@[simp_scalar_safe]` it is better to group all the preconditions into a single one,
