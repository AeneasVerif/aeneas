...
public meta import AeneasMeta.Saturate.Tactic
...
meta def unexpForall' : Lean.PrettyPrinter.Unexpander | `($_ $_) => `(∀ _, __) | _ => throw ()
...
meta def omegaTacticNoAuxThm (cfg : Meta.Omega.OmegaConfig) : TacticM Unit := do
...
meta def scalarTacSaturateForward {α}
...
meta def getSimpThmNames : CoreM (Array Name) := do
...
meta def simpAsmsTarget (simpOnly : Bool) (config : Simp.Config) (args : Simp.SimpArgs) : TacticM (Option (Array FVarId)) :=
  withMainContext do
...
meta def preprocess (config : Config) : Tactic.TacticM Unit := do
...
meta def State.new (config : Config) : MetaM State := do
  let env ← getEnv
  let rules :=
    if config.nonLin then #[scalarTacAttribute, scalarTacNonLinAttribute]
    else #[scalarTacAttribute]
...
meta def partialPreprocess (config : Config) (simpArgs : Simp.SimpArgs) (state : State)
...
meta def scalarTacCore (config : Config) : Tactic.TacticM Unit := do
...
-/
meta def scalarTacAux (config : Config) : TacticM Unit := do
...
meta def scalarTac (config : Config) : TacticM Unit := do
  Tactic.focus do
...
meta def incrScalarTacCore (config : Config) (state : State) (toClear : Array FVarId) (assumptions : Array FVarId) : TacticM Unit := do
  Tactic.focus do
  Tactic.withMainContext do
...
/-- Incremental version of `scalar_tac`, where we call preprocessing several times to incrementally
 ...
 -/
meta def incrScalarTac (config : Config) (state : State) (toClear : Array FVarId) (assumptions : Array FVarId) : TacticM Unit := do
