...
def matchBind? (e : Expr) : Option (Expr × Expr × Expr) := do
  let fn := e.getAppFn
...
private def mkBind (computation continuation : Expr) : MetaM Expr := do
...
private def mkPure (m value : Expr) : MetaM Expr := do
...
private partial def tuplePatternTree? (type : Expr) (fields : Array Expr) (idx : Nat) :
...
private def normalizeTupleBinds (e : Expr) : MetaM Expr :=
...
private theorem resultBind_eq {α β : Type u}
    (x : Aeneas.Std.Result α) (f : α → Aeneas.Std.Result β) :
    x >>= f = Aeneas.Std.bind x f := rfl

private theorem resultPure_eq {α : Type u} (x : α) :
