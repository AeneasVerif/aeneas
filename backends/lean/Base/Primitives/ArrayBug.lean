...
theorem Slice.index_simp {α : Type u} [Inhabited α]
  (val : List α) (property : (fun l => l.length ≤ Usize.max) val) (i: Int) :
...
@[simp]
theorem Slice.slice_simp {α : Type u} [Inhabited α]
  (val : List α) (property : (fun l => l.length ≤ Usize.max) val) (i j : Int) :
  Slice.slice ⟨ val, property ⟩ i j = val.slice i j := by simp
...
--  (∀ i, r.end_.val ≤ i → i < a.length → na.index i = a.index i)
  := by
  simp [mut_subslice_back]
  simp [*]
