...
and eval_local_function_call_concrete (config : C.config) (fid : E.FunDeclId.id)
...
and eval_local_function_call_symbolic (config : C.config) (fid : E.FunDeclId.id)
    (region_args : T.erased_region list) (type_args : T.ety list)
    (args : E.operand list) (dest : E.place) : st_cm_fun =
 fun cf ctx ->
...
(** Evaluate a local (i.e, not assumed) function call (auxiliary helper for
    [eval_statement]) *)
and eval_local_function_call (config : C.config) (fid : E.FunDeclId.id)
    (region_args : T.erased_region list) (type_args : T.ety list)
    (args : E.operand list) (dest : E.place) : st_cm_fun =
  match config.mode with
  | ConcreteMode ->
