...
let compute_typed_avalue_proj_kind span type_infos (av : V.typed_avalue) :
    typed_avalue_kind =
  ...
  let keep_region (r : T.region) =
    match r with
    | T.RVar (Free rid) ->
        sanity_check __FILE__ __LINE__ (rid = Types.RegionId.zero) span;
        true
    | _ -> false
  in
  let visitor =
...
let rec typed_avalue_to_consumed_aux ~(filter : bool) (ctx : bs_ctx)
  ...
  let value =
    ...
    | ASymbolic (pm, aproj) ->
        ...
        aproj_to_consumed_aux ctx aproj av.ty
    | AIgnored mv -> (
        if filter then None
        else
...
and adt_avalue_to_consumed_aux ~(filter : bool) (ctx : bs_ctx)
  ...
  match compute_typed_avalue_proj_kind ctx.span ctx.type_ctx.type_infos av with
...
and aproj_to_consumed_aux (ctx : bs_ctx) (aproj : V.aproj) (ty : T.ty) :
    texpression option =
  match aproj with
...
let typed_avalue_to_consumed (ctx : bs_ctx) (ectx : C.eval_ctx)
  ...
  | LoanProj BMut ->
      ...
      typed_avalue_to_consumed_aux ~filter:true ctx ectx av
...
(** Explore an abstraction value which we know **was generated by a borrow
    ...
    - the pattern *)
let rec typed_avalue_to_given_back_aux ~(filter : bool) (mp : mplace option)
...
let typed_avalue_to_given_back (mp : mplace option) (v : V.typed_avalue)
  ...
  (* Check if the value was generated from a borrow projector: if yes, and if
     ...
     Otherwise we ignore it. *)
  match compute_typed_avalue_proj_kind ctx.span ctx.type_ctx.type_infos v with
