...
let rec tavalue_to_consumed_ty_aux ~(filter : bool) (ctx : bs_ctx)
    ...
    (current_level : abs_level) (av : V.tavalue) : ty option =
  match av.value with
  | AAdt adt_v ->
      adt_avalue_to_consumed_ty_aux ~filter ctx abs_regions abs_level
  ...
  let _, out =
    gtranslate_adt_fields ~project_borrows:false (tavalue_to_string ctx)
      ...
      (fun ~filter ctx v ->
        ( ctx,
          match
            tavalue_to_consumed_ty_aux ~filter ctx abs_regions abs_level
              current_level v
          with
          | None -> None
          | Some x -> Some ((), x) ))
...
let tavalue_to_consumed_ty (ctx : bs_ctx) (abs_regions : T.RegionId.Set.t)
    (abs_level : abs_level) (av : V.tavalue) : ty option =
  ...
  | LoanProj BMut ->
      ...
      tavalue_to_consumed_ty_aux ~filter:true ctx abs_regions abs_level 0 av
...
let rec tavalue_to_given_back_ty_aux ~(filter : bool) (ctx : bs_ctx)
  ...
  match av.value with
  | AAdt adt_v ->
      adt_avalue_to_given_back_ty_aux ~filter abs_regions abs_level
        current_level av adt_v ctx
  | ALoan _ ->
  ...
  let _, out =
    gtranslate_adt_fields ~project_borrows:true (tavalue_to_string ctx)
      ...
      (fun ~filter ctx v ->
        ( ctx,
          match
            tavalue_to_given_back_ty_aux ~filter ctx abs_regions abs_level
              current_level v
          with
          | None -> None
          | Some x -> Some ((), x) ))
...
let tavalue_to_given_back_ty (ctx : bs_ctx) (abs_regions : T.RegionId.Set.t)
    (abs_level : abs_level) (v : V.tavalue) : ty option =
  [%cassert] ctx.span (abs_level = 0) "Unimplemented";
  ...
  with
  | BorrowProj BMut ->
      tavalue_to_given_back_ty_aux ctx abs_regions abs_level 0 ~filter:true v
...
let abs_to_input_output_tys (ctx : bs_ctx) (abs : V.abs) : ty list * ty list =
  (* TODO: properly handle the sub-abstractions *)
  let abs_level = 0 in
  let inputs =
    List.filter_map
      (tavalue_to_consumed_ty ctx abs.regions.owned abs_level)
      abs.avalues
  in

  let outputs =
    List.filter_map
      (tavalue_to_given_back_ty ctx abs.regions.owned abs_level)
      abs.avalues
  in
...
let compute_tevalue_proj_kind (span : Meta.span) (type_infos : type_infos)
    (abs_regions : T.RegionId.Set.t) (abs_level : abs_level)
    (current_level : abs_level) (ev : V.tevalue) : proj_kind =
...
let eoutput_to_pat (ctx : bs_ctx) (fvar_to_texpr : texpr V.AbsFVarId.Map.t ref)
    (rids : T.RegionId.Set.t) (abs_level : abs_level) (output : V.tevalue) :
  ...
  let add_concrete ctx (bid : V.BorrowId.id) (ty : T.ty) : bs_ctx * tpat =
    ...
    (ctx, pat)
  in
  let rec to_pat ~(filter : bool) (current_level : abs_level) (ctx : bs_ctx)
      (output : V.tevalue) : bs_ctx * tpat option =
    ...
    | V.EAdt { variant_id; fields } -> begin
        let ctx, out =
          gtranslate_adt_fields ~project_borrows:true (tevalue_to_string ctx)
            (tpat_to_string ctx)
            (fun ~filter ctx v ->
              let ctx, pat = to_pat ~filter current_level ctx v in
              ( ctx,
                match pat with
                | None -> None
                | Some x -> Some ((), x) ))
...
let einput_to_texpr (ctx : bs_ctx) (ectx : C.eval_ctx) ?(to_consumed = false)
  ...
  and to_texpr ~(filter : bool) (rids : T.RegionId.Set.t) (ctx : bs_ctx)
    ...
    let e = to_texpr_aux ~filter rids 0 ctx input in
    [%ldebug
      let _, _, e = e in
      "- rids: "
      ^ T.RegionId.Set.to_string None rids
...
let rec tevalue_to_given_back_aux ~(filter : bool)
    (abs_regions : T.RegionId.Set.t) (abs_level : abs_level)
    (current_level : abs_level) (mp : mplace option) (ev : V.tevalue)
    (ctx : bs_ctx) : bs_ctx * tpat option =
  let (ctx, value) : _ * tpat option =
    match ev.value with
    | EAdt adt_v ->
        adt_evalue_to_given_back_aux ~filter abs_regions abs_level current_level
          ev adt_v ctx
    | ELoan _ ->
...
and adt_evalue_to_given_back_aux ~(filter : bool)
    (abs_regions : T.RegionId.Set.t) (abs_level : abs_level)
    (current_level : abs_level) (av : V.tevalue) (adt_v : V.adt_evalue)
    (ctx : bs_ctx) : bs_ctx * tpat option =
  let ctx, out =
    gtranslate_adt_fields ~project_borrows:true (tevalue_to_string ctx)
      (tpat_to_string ctx)
      (fun ~filter ctx v ->
        let ctx, v =
          tevalue_to_given_back_aux ~filter abs_regions abs_level current_level
            None v ctx
        in
        match v with
        | None -> (ctx, None)
...
let tevalue_to_given_back (abs_regions : T.RegionId.Set.t)
    (abs_level : abs_level) (mp : mplace option) (v : V.tevalue) (ctx : bs_ctx)
    : bs_ctx * tpat option =
  (* Check if the value was generated from a borrow projector: if yes, and if
     it contains mutable borrows we generate a given back pattern (because
     upon ending the borrow the abstraction gave back a value).
  ...
  with
  | BorrowProj BMut ->
      tevalue_to_given_back_aux abs_regions abs_level 0 mp ~filter:true v ctx
  | BorrowProj BShared | LoanProj _ | UnknownProj ->
      (* If it is a loan proj we ignore it. If it is an unknown projection,
         it means the value doesn't contain loans nor borrows, so nothing
         is given back: we can ignore it as well. *)
...
let ended_abs_cont_to_texpr_aux (ctx : bs_ctx) (ectx : C.eval_ctx) (abs : V.abs)
    (abs_level : abs_level) (output : V.tevalue) (input : V.tevalue) :
    bs_ctx * bool * tpat * texpr =
  let span = ctx.span in
...
let translate_ended_abs_to_texpr (ctx : bs_ctx) (ectx : C.eval_ctx)
    (abs : V.abs) (abs_level : abs_level) : bs_ctx * bool * tpat * texpr =
  [%ltrace "abs:\n" ^ abs_to_string ctx abs];
  match abs.cont with
  | None -> [%internal_error] ctx.span
  | Some cont -> (
