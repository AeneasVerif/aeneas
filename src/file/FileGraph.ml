...
let bucket_of_item (crate : crate) ~(on_foreign_span : string -> unit)
  ...
  | Some meta ->
      if not meta.is_local then
        ...
        if file.crate_name = crate.name then Some (BFile path)
        else (
          ...
          Some (BFile (source_path_of_name meta.name)))
...
let compute (crate : crate) : t =
  ...
  let item_bucket : bucket AnyDeclIdMap.t =
    List.fold_left
      (fun acc id ->
        match bucket_of_item crate ~on_foreign_span id with
        | None ->
