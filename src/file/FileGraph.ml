  ...
  | Some meta ->
      if not meta.is_local then
        ...
        if file.crate_name = crate.name then (
          ...
          (* An item generated by a `macro_rules!` macro from another crate is
             local, but its span points at the macro *definition* — a file of
             the defining crate, which may not even appear in this crate's file
             table.

             That is charon behaving consistently, not a bug: a span says where
             the source text lives, and it never resolves macro expansions to
             their call site ([generated_from_span] tracks MIR inlining, not
             expansion). It is the right answer for an error message or a
             [Source:] comment. It is the wrong answer for us, because we are
             asking a different question — which file *owns* this item — and
             taking the span at face value files it under a foreign path,
             emitting a module that mirrors the other crate's tree.

             So fall back to the Rust module path, which is always local and,
             under the usual module-to-file convention, names the file the macro
             was invoked from. That convention is not universal ([#[path]],
             inline [mod] blocks, and [mod.rs] all break it), so the item may
             land in a module of its own rather than merging with its
             file-mates — still a local, sensible tree, unlike the
             alternative. *)
