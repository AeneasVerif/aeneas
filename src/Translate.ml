...
(** Per-file extraction ([-split-files]): emit one Lean module per source file,
    merging files that form an import cycle into a single module, and splitting
    each module into opacity layers (regenerated [PartK] files and user-fillable
...
let extract_by_file (ctx : gen_ctx) (crate : crate) ~(dest_dir : string)
  ...
  (* The first line of every generated Lean file; the entry-point overwrite
     guard below keys on it. *)
  let generated_header = "-- THIS FILE WAS AUTOMATICALLY GENERATED BY AENEAS" in
  ...
  (* Emit one module per SCC, in topological (dependency-first) order; each
     module is a chain of one or more opacity layers. *)
  List.iter
    (fun (m : FilePlan.placed_module) ->
      if m.is_dropped then ()
      else
        let scc_id = m.scc_id in
        (* Imports of the module's first layer: the plain module names of the
           SCCs this one depends on, skipping any dropped (empty builtin-only
           external) module. *)
        let deps =
          Option.value
        ...
        in
        let merged_msg =
          match merged_srcs with
          | Some srcs -> ": merged from " ^ srcs
          | None -> ""
        in
        (* Emit the layer chain. Layer k>1 imports layer k-1 — by its *filled*
           name for a template — and Lean imports are transitive, so earlier
           layers and all module dependencies stay visible. The fold threads
           each layer's import name as the next layer's includes. *)
        let (_ : string list) =
          List.fold_left
            (fun custom_includes (layer : FilePlan.placed_layer) ->
              let config = { base_config with interface = layer.is_template } in
              let custom_msg =
                if layer.is_template then
              ...
              let file_info =
                {
                  ...
                  (* External modules don't open the crate namespace: their
        ...
        in
        (* The aggregator: an imports-only file at the module's plain name, so
           dependents and the entry point can import [Crate.Foo] whether or
           not the module is split into layers. It imports templates by their
           filled names, so building it requires the templates to be
           copied/renamed first. *)
        match m.aggregator with
        | None -> ()
        | Some filename ->
            write_imports_file ~filename
              ~custom_msg:
                (": imports this module's opacity layers (regenerated [PartK] \
                  files and user-filled [AxiomsK] templates)"
                ^
                match merged_srcs with
                | Some srcs -> "\n-- Merged from " ^ srcs
                | None -> "")
              ~imports:
                (List.map
                   (fun (l : FilePlan.placed_layer) -> l.import_name)
                   m.layers))
    placed;
  ...
  if Option.is_none !Config.subdir then (
    ...
    (* A hand-written file at this path (e.g. a root file kept from a previous
       [-split-files] layout) carries no generated header: refuse to clobber it
       instead of silently destroying user work. Generated entry points (which
       do carry the header) are regenerated as usual. *)
