(** Placement plan for [-split-by-file]: which Lean module/file each SCC of the
    file-dependency graph becomes. Used by the the emitter
    ({!Translate.extract_by_file}) and the [-dump-file-graph] diagnostic. *)

open FileGraph

(** One generated Lean module in [-split-by-file] mode. *)
type placed_module = {
  scc_id : SCC.SccId.id;
...
(** Render the generated-files section of the [-dump-file-graph] report: exactly
    the files the [-split-by-file] emitter will write (dropped builtin-only
...
let render_placement (placed : placed_module list) : string =
  ...
  in
  line "================ GENERATED FILES (-split-by-file) ================";
