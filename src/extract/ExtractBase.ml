...
let builtin_adts () : (builtin_ty * string) list =
  ...
  | HOL4 ->
      (* HOL4 uses the prover's native sum type and sumSyntax tooling for this
         ...
         the way they do in Lean/Coq/F*. Nested-loop HOL4 output is not
         regenerated by CI today; the [control_flow]/[loop_exit] entries below
         reserve the shared extraction names for when the HOL4 primitive side is
         enabled. *)
      [
        (TResult, "result");
        (TSum, "sum");
...
let builtin_variants () : (builtin_ty * VariantId.id * string) list =
  ...
  | HOL4 ->
      [
        (TResult, result_ok_id, "Ok");
        (TResult, result_fail_id, "Fail");
        (TSum, sum_left_id, "INL");
        (TSum, sum_right_id, "INR");
        (TLoopResult, loop_result_continue_id, "Cont");
        (TLoopResult, loop_result_break_id, "Done");
        (TLoopExit, loop_exit_normal_break_id, "NormalBreak");
        (TLoopExit, loop_exit_propagated_break_id, "PropagatedBreak");
