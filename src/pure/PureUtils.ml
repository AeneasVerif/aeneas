...
type regular_fun_id_not_loop = FunDeclId.id * RegionGroupId.id option
[@@deriving show, ord]
...
(** Check whether the failure of the *monadic* expression [e], which is
    immediately followed by [cont], is subsumed by the failure of [cont]: if [e]
    fails then [cont] fails as well.

    Together with the fact that the values bound by [e] are not used in [cont],
    this makes it sound to remove [let _ ← e; cont] from [let _ ← e; cont].
    ...
    Such pairs are systematically generated by [rustc] for the nested place
    expressions which appear on the left-hand side of an assignment (e.g.,
    [v[i][j] = x]): the shared access is emitted for the benefit of the borrow
    checker only (it feeds a [FakeRead]) and has no runtime meaning. *)
let texpr_failure_subsumed_by_cont (e : texpr) (cont : texpr) : bool =
  (* Retrieve the expression which is evaluated right after [e]: either the
     expression bound by the first let-binding of [cont], if [cont] is a
     *monadic* let-binding, or [cont] itself if it is a terminal call.

     Note that we must ignore the non-monadic let-bindings: they bind pure
     expressions, meaning a failure appearing inside of them is not propagated
     but simply bound to a variable. *)
  let next =
    match cont.e with
