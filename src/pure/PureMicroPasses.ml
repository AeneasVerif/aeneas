...
let decompose_let_bindings_visitor (decompose_monadic : bool)
  ...
  let mk_fresh (ty : ty) : tpattern * texpr =
    ...
    let ltmp = mk_tpattern_from_fvar None tmp in
      ...
      *)
      ...
      if not monadic then super#visit_Let env monadic lv re e
        ...
        *)
        ...
        in
        let err_pat = mk_tpattern_from_fvar None err_var in
...
let add_fuel_and_state_one (ctx : ctx) (loops : fun_decl LoopId.Map.t)
  ...
  let update_call (f : texpr) (args : texpr list) (fuel : texpr option)
    ...
    let state'_pat = Option.map (mk_tpattern_from_fvar None) state' in
    let e =
        ...
        begin
          ...
          | _ ->
              ...
              if re_is_stateful then
                ...
                let state'_pat = mk_tpattern_from_fvar None state' in
  ...
  let body =
    Option.map
      (fun (body : fun_body) ->
        ...
        (* Update the inputs *)
        let inputs =
          let fuel_pat = Option.map (mk_tpattern_from_fvar None) fuel_input in
          let state_pat = Option.map (mk_tpattern_from_fvar None) state in
  ...
  let update_loop_body (keep_outputs : (bool * FVarId.id option) list)
    ...
    *)
    let rebind_loop (loop : loop) : texpr =
      ...
      let pats = List.map (mk_tpattern_from_fvar None) fvars in
      let pat = mk_simpl_tuple_pattern pats in
        ...
        | Loop loop ->
            ...
            let new_pats =
              List.map
                ...
                fresh_outputs
            in
            let new_pats = List.map (mk_tpattern_from_fvar None) new_pats in
            ...
            *)
            ...
            let pat =
              mk_simpl_tuple_pattern
                (List.map (mk_tpattern_from_fvar None) kept_output_vars)
            in
            mk_closed_let span monadic pat loop next
        | _ ->
            (* No need to update the bound expression *)
...
let filter_prefix file line span (keep : bool list) (ls : 'a list) : 'a list =
  let ls0, ls1 =
    try Collections.List.split_at ls (List.length keep)
    with Failure _ ->
      Errors.craise file line span "Internal error: please file an issue"
  in
  let ls0 =
    List.filter_map
      (fun (b, x) -> if b then Some x else None)
...
let filter_loop_inputs_explore_one_visitor (ctx : ctx)
  ...
  let used =
    ref
      (List.map
         (fun v -> ((fst ([%add_loc] as_pat_open span v)).id, false))
         body.inputs)
  in
  let inputs =
    List.map
      (fun v ->
        let v, _ = [%add_loc] as_pat_open span v in
  ...
  let inputs_set =
    FVarId.Set.of_list
      (List.map
         (fun x ->
           let x, _ = [%add_loc] as_pat_open span x in
           x.id)
         body.inputs)
  in
...
let filter_loop_inputs_filter_in_one (_ctx : ctx)
  ...
  let decl =
    ...
    | Some used_info ->
        let {
          ...
          decl.signature
        in
        let inputs = [%add_loc] filter_prefix span used_info inputs in
  ...
  let filter_in_body (body : fun_body) : fun_body =
              ...
              | Qualif qualif -> (
                  ...
                  | FunOrOp (Fun (FromLlbc (FunId fun_id, loop_id))) -> (
                      ...
                      | Some used_info ->
                          ...
                          let tys =
                            [%add_loc] filter_prefix span used_info tys
                          in
                          ...
                          let args =
                            [%add_loc] filter_prefix span used_info args
                          in
    ...
    { inputs; body }
  in
  map_open_all_fun_decl_body filter_in_body decl
