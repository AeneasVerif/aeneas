...
let () =
  ...
  | Ok m ->
      ...
      let check_cli_option (b : bool) : unit =
        check_no_doc b
          "Invalid option detected: the serialized crate was generated by \
           Charon without the `--preset=aeneas` option. Please regenerate the \
           crate by calling Charon with `--preset=aeneas`.\n"
      in
      check_cli_option m.options.hide_marker_traits;
      check_cli_option (m.options.remove_associated_types = [ "*" ]);
