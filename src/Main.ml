...
let () =
  ...
  | Ok m ->
      ...
      let check_cli_option (b : bool) (option : string) : unit =
        check_no_doc b
          ("Invalid option detected: the serialized crate was generated by \
            Charon without the `" ^ option
         ^ "` option. Please regenerate the crate by calling Charon with this \
            option.\n")
      in
      check_cli_option m.options.hide_marker_traits "--hide-marker-traits";
      check_cli_option
        (m.options.remove_associated_types = [ "*" ])
