...
class ['self] map_abs_expr_base =
  object (_self : 'self)
    ...
    method visit_loop_abs_kind : 'env -> loop_abs_kind -> loop_abs_kind =
      fun _ x -> x
...
and abs_fun =
  ...
  | FunCall of fun_call_id * region_group_id
      (** The expression is the result of calling the backward function of a
          ...
          symbolic execution: this is only used for pretty-printing and
          debugging in the symbolic AST, generated by the symbolic execution. *)
  | Loop of (loop_id * region_group_id option * loop_abs_kind)
      (** The expression is the result of calling the backward function of a
          loop.
...
(** The continuation representating the computation performed that has to be
    performed when ending a region abstraction (this continuation computes the
    values given back by the abstraction from the values it consumed).
...
class ['self] iter_abs_base =
  object (_self : 'self)
    ...
    method visit_abs_cont : 'env -> abs_cont -> unit = fun _ _ -> ()
  end
...
class ['self] map_abs_base =
  object (_self : 'self)
    ...
    method visit_abs_cont : 'env -> abs_cont -> abs_cont = fun _ x -> x
  end
...
type abs = {
  ...
  cont : abs_cont option;
      (** The continuation representating the abstraction in the translation.
