            ...
            let e =
              if sv_id = original_sv.sv_id then (
                      ...
                      List.map
                        (fun (sv : symbolic_value) ->
                          ({
                             ...
                             ty = normalize_proj_ty abs.regions.owned sv.sv_ty;
                           }
                            : abs_texpr))
                        fields
                    in
                ...
                | SeMutRef (bid, sv) ->
                    ...
                    if region_in_set r abs.regions.owned then ELoan bid
                    else
                      (* TODO: not sure if we should put the shared value, unit, or a
                         special "ignore" value *)
                      abs_expr_unit
                | SeSharedRef (_, sv) ->
                    (* TODO: we need to do things differently with nested borrows *)
                    cassert __FILE__ __LINE__
            ...
            let opat =
              if sv_id = original_sv.sv_id then (
                ...
                | SeAdt (variant_id, fields) ->
                    ...
                    let fields =
                      List.map
                        (fun (sv : symbolic_value) ->
                          ({
                             opat = OSymbolic sv.sv_id;
                             opat_ty =
                               normalize_proj_ty abs.regions.owned sv.sv_ty;
                           }
                            : abs_toutput))
                        fields
                    in
                ...
                | SeMutRef (bid, sv) ->
                    ...
                    if region_in_set r abs.regions.owned then OBorrow bid
                    else
                      (* TODO: not sure if we should put the shared value, unit, or a
                         special "ignore" value *)
                      abs_output_unit
                | SeSharedRef (_, sv) ->
                    ...
                    cassert __FILE__ __LINE__
                      (not (symbolic_value_has_borrows (Some span) ctx sv))
                      span "Nested borrows are not supported yet";
                    (* TODO: not sure if we should put the shared value, unit, or
                       a special "ignore" value *)
                    abs_output_unit)
  ...
  let ctx_visitor =
    object
      ...
      method! visit_abs _ abs =
        {
          abs with
          cont = Option.map (cont_visitor#visit_abs_cont abs) abs.cont;
        }
    end
  in
