...
let name_with_generics_to_simple_name (ctx : trans_ctx)
    ?(keep_var_generics = false) ?(prefix : Types.name option = None)
    (name : Types.name) (p : Types.generic_params) (g : Types.generic_args) :
    string list =
  let mctx = Charon.NameMatcher.ctx_from_crate ctx.crate in
  name_with_generics_to_simple_name mctx ~keep_var_generics ~prefix name p g
...
let trait_name_with_generics_to_simple_name (ctx : trans_ctx)
    ?(keep_var_generics = false) ?(prefix : Types.name option = None)
    (n : Types.name) (p : Types.generic_params) (g : Types.generic_args) :
    string list =
  name_with_generics_to_simple_name ctx ~keep_var_generics ~prefix n p g
