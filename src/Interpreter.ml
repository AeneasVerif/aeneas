...
(** Initialize an evaluation context to execute a global.
    Perhaps refactor with [initialize_symbolic_context_for_function].
 *)
let initialize_symbolic_context_for_global
    (type_context : C.type_context)
    (fun_context : C.fun_context)
    (global_context : C.global_context)
    (gdef : A.global_decl) :
    C.eval_ctx =
  (* Split the variables between return var, inputs and remaining locals *)
  let body = Option.get gdef.body in
  assert (body.arg_count == 0);
  let ret_var = List.hd body.locals in
  let local_vars = List.tl body.locals in
...
(** Evaluate a global with the symbolic interpreter.
    Perhaps refactor with [evaluate_function_symbolic].
 *)
let evaluate_global_symbolic (config : C.partial_config) (synthesize : bool)
    (type_context : C.type_context) (fun_context : C.fun_context) (global_context : C.global_context)
    (gdef : A.global_decl) :
    SA.expression option =
  (* Debug *)
  let name_to_string () =
  Print.global_name_to_string gdef.A.name
  in
  log#ldebug (lazy ("evaluate_global_symbolic: " ^ name_to_string ()));

  (* Create the evaluation context *)
  let ctx = initialize_symbolic_context_for_global type_context fun_context global_context gdef in
  ...
  | Panic ->
      ...
      if synthesize then Some SA.Panic else None
  | _ ->
      failwith ("evaluate_global_symbolic failed on: " ^ name_to_string ())
  in

  (* Evaluate the global *)
  eval_function_body config (Option.get gdef.body).body cf_finish ctx
