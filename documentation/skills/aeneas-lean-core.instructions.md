...
## Context
Aeneas translates Rust programs to pure Lean code via the LLBC intermediate representation. The generated code uses the `RustM` error monad. Proofs verify functional correctness by writing specification theorems tagged with `@[step]`.
...
- `Funs.lean` — Rust functions → Lean monadic functions (RustM monad)
...
- Arithmetic operations return `RustM` (may fail on overflow)

### Translation patterns
| Rust | Lean |
|---|---|
| `&mut T` param | `T` param (by value) |
| Return `&'a mut T` | Returns `RustM (T × (T → ...))` — backward continuation |
...
- **Do NOT use `partial`** — `partial` definitions are opaque and cannot be unfolded or
  reasoned about. If the function lives in `RustM`, use `partial_fixpoint` instead.
...
`RustM (Error × Output)`), the postcondition must use an **exhaustive `match`** on
...
Pure (i.e., not living in `RustM`) Rust global/const scalar definitions should be
