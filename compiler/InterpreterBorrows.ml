...
(** Small utility which we use to compute meta-information that is used to find
    proper names for the values given back by abstractions.

    When we end an abstraction, this abstraction outputs some symbolic values
    ...
    As [x] maps directly to [s0], it makes sense to give the name "x" to this
    symbolic value, in the generated translation.

    Rem.: this is actually redundant with the fact that we propagate the
    meta-place information from the forward inputs to the backward inputs
    (see {!SymbolicToPure.abs_to_given_back} which takes optional places
    as parameter).

    Parameters:
    - [ctx]: the context just after we removed the abstraction
    - [abs]: the ended abstraction, that we just removed from the context
    - [e]: the symbolic expression generated by continuing the execution with
      context [ctx]
 *)
let add_naming_hints_for_abstraction_given_back_values (ctx : C.eval_ctx)
  ...
  let svs =
    V.SymbolicValueId.Set.of_list
      (List.map (fun (v : V.symbolic_value) -> v.sv_id) svl)
  in
    ...
    let visitor =
      object (self)
        ...
        method! visit_env_elem _ ee =
          match ee with
          | C.Var (C.VarBinder var, value) ->
              super#visit_typed_value (Some var) value
          | Var (C.DummyBinder _, _) | Abs _ | Frame -> (* Ignore *) ()

        method! visit_typed_value env v =
          match v.V.value with
          | Adt av -> (
              (* We dive inside boxes or tuples with only one field *)
              ...
              | [ fv ] -> (
                  ...
                  | Tuple | Assumed Box ->
                      (* Dive *)
                      self#visit_typed_value env fv
                  | AdtId _ | Assumed (Vec | Option) -> ())
              | _ -> ())
          | Borrow bc -> (
              match bc with
              | V.MutBorrow (_, mv) ->
                  (* Dive into mut borrows *)
                  self#visit_typed_value env mv
              | V.SharedBorrow _ | V.ReservedMutBorrow _ -> ())
          | Loan _ ->
              (* Do nothing: we can't dive into mut loans, and there is no point
                   in diving into shared loans: shared values can't be mutably
                   borrowed
              *)
              ()
          | Symbolic sv ->
              (* Check if this symbolic value was given back by the abstraction *)
              if V.SymbolicValueId.Set.mem sv.sv_id svs then
                meta_ls := (Option.get env, sv) :: !meta_ls
              else ()
  ...
  List.fold_right
    (fun (var, v) e -> S.mk_meta (SA.SymbolicAtPlace (var, v)) e)
    meta_ls e
...
and end_abstraction_remove_from_context (_config : C.config)
 ...
 fun cf ctx ->
  ...
  (* Apply the continuation *)
  let expr = cf ctx in
  (* Add the meta information to help with finding pretty names *inside*
     tne [EndAbstraction] node *)
  let expr =
    Option.map (add_naming_hints_for_abstraction_given_back_values ctx abs) expr
  in
  (* Synthesize the [EndAbstraction] *)
