...
let mk_expr_result_fail_texpression (meta : Meta.meta) (error : texpression)
    (ok_ty : ty option) (return_ty : ty option) : texpression =
  match (ok_ty, return_ty) with
  | Some ok_ty, None -> mk_result_fail_texpression meta error ok_ty
  | _ ->
...
let mk_expr_result_fail_texpression_with_error_id (meta : Meta.meta)
    (error : VariantId.id) (ok_ty : ty option) (return_ty : ty option) :
    texpression =
  let error = mk_error error in
  mk_expr_result_fail_texpression meta error ok_ty return_ty
...
let mk_expr_result_ok_texpression (meta : Meta.meta) (v : texpression)
...
let mk_expr_result_return_texpression (meta : Meta.meta) (ok_ty : ty option)
    (v : texpression) : texpression =
...
(** Destruct an [ok ...] expression *)
let opt_destruct_ok (e : texpression) : texpression option =
  match e.e with
  | App
      ( {
          e =
