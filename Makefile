...
tests: trans-no_nested_borrows trans-paper \
	...
	trans-loops \
  trans-arrays # TODO: Generalize to all backends
...
trans-arrays: OPTIONS += -no-state
trans-arrays: SUBDIR := misc
tlean-arrays: SUBDIR :=
tfstar-arrays: OPTIONS += -decreases-clauses -template-clauses
tcoq-arrays: OPTIONS += -use-fuel -no-split-files
thol4-arrays: SUBDIR := misc-arrays
...
.PHONY: tlean-%
tlean-%: CHARON_TEST_DIR = $(CHARON_TESTS_REGULAR_DIR)
